...
class Improved_Phi_GRU_ATT_Streaming(nn.Module):
    ...
    def export_onnx(self, save_path, input_shape=(1, 64, 50), simplify_model=True):
        """
        Optimized ONNX export method with optional simplification
        """
        ...
        try:
            ...
            torch.onnx.export(
                ...
                (spectrogram_input, h0_input),  # model inputs (on CPU)
            ...
            ort_outs = ort_session.run(
                ['output_logits', 'hidden_state_output'],
                ort_inputs
            )
            print("ONNX Runtime test passed.")
            print(f"Output shapes: {ort_outs[0].shape}, Hidden state: {ort_outs[1].shape}")
...
def main(cfg: DictConfig):
    ...
    if exported_model_path:
        ...
        ort_outputs = ort_session.run(["output_logits", "hidden_state_output"], ort_inputs)

        logits, new_h = ort_outputs
