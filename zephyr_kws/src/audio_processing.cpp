 ...
 * 
 * audio_processing.cpp - Real-time audio processing pipeline.
...
int16_t AudioProcessor::process_sample_pair(int16_t raw_l, int16_t raw_r)
{
    ...
    const float input_gain = 0.50f; // Boost sensitivity for 2.0m far-field speech
    const float in_limit = 16384.0f; // 6dB of headroom for close-up speech
    ...
    // --- 2. DC Blocking HPF ---
    const float hpf_r = 0.968f; // ~80 Hz cutoff (blocks low-frequency hum, power line noise, and wind pops)
...
void AudioProcessor::check_and_report_diagnostics()
{
    ...
    if (diag_sample_count >= diag_report_interval) {
        ...
        if (rms_l < 10.0f && rms_r < 10.0f) {
            ...
            LOG_WRN("[DIAGNOSIS] Active audio is on RIGHT channel (CH1), but KWS is reading LEFT! Check mic channel strapping.");
        }
